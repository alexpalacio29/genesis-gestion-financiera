    ...
    try {
      ...
      const payload = {
        ...
        check: {
          ...
          date: metadata.date,
          amount_gross: hasTaxWithholding ? total / 2 : total,
          subtotal: hasTaxWithholding ? subtotal / 2 : subtotal,
          itbis_total: hasTaxWithholding ? itbis / 2 : itbis,
          retention_isr: hasTaxWithholding ? 0 : retention_isr,
          retention_itbis: hasTaxWithholding ? retention_itbis : retention_itbis,
          amount_net: hasTaxWithholding ? (total / 2) - retention_itbis : amount_net,
          beneficiary: previewData.supplier.name,
          description: hasTaxWithholding ? `Pago 50% (Retención 100% ITBIS) - ${finalDescription}` : finalDescription
        },
        checkTax: hasTaxWithholding ? {
          check_number: metadata.checkNumberTax || 'CH-' + Math.floor(Math.random() * 10000),
          date: metadata.date,
          amount_gross: total / 2,
          subtotal: subtotal / 2,
          itbis_total: itbis / 2,
          retention_isr: retention_isr,
          retention_itbis: 0,
          amount_net: (total / 2) - retention_isr,
          beneficiary: previewData.supplier.name,
          description: `Pago 50% (Retención 5% ISR) - ${finalDescription}`
        } : null,
        bank_transaction: {
          type: 'expense',
          amount: amount_net,
